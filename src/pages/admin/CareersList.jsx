  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setEditing({ _isNew: true, id: "", title: "", department: "", location: "", type: "", description: "", active: true, sortOrder: 0 })} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-accent text-white text-sm font-semibold hover:bg-blue-600 transition">
      ...
      {editing && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center">
          ...
          <div className="relative bg-space-800 border border-[#2a2a2a] rounded-2xl p-6 max-w-lg w-full mx-4 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between">
            ...
            <form onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.target); saveMutation.mutate({ ...editing, id: editing._isNew ? fd.get("id") : editing.id, title: fd.get("title"), department: fd.get("department"), location: fd.get("location"), type: fd.get("type"), description: fd.get("description"), active: fd.get("active") === "true", sortOrder: Number(fd.get("sortOrder") || 0) }); }}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-300 mb-1">Department</label><input name="department" defaultValue={editing.department} className="w-full px-4 py-2.5 rounded-xl bg-space-900 border border-[#2a2a2a] text-white text-sm focus:border-accent focus:outline-none transition" /></div>
                <div><label className="block text-sm font-medium text-gray-300 mb-1">Location</label><input name="location" defaultValue={editing.location} className="w-full px-4 py-2.5 rounded-xl bg-space-900 border border-[#2a2a2a] text-white text-sm focus:border-accent focus:outline-none transition" /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-gray-300 mb-1">Type</label><input name="type" defaultValue={editing.type} className="w-full px-4 py-2.5 rounded-xl bg-space-900 border border-[#2a2a2a] text-white text-sm focus:border-accent focus:outline-none transition" placeholder="Full-time, Part-time..." /></div>
                <div><label className="block text-sm font-medium text-gray-300 mb-1">Status</label><select name="active" defaultValue={String(editing.active)} className="w-full px-4 py-2.5 rounded-xl bg-space-900 border border-[#2a2a2a] text-white text-sm focus:border-accent focus:outline-none transition"><option value="true">Active</option><option value="false">Inactive</option></select></div>
              </div>
              <div><label className="block text-sm font-medium text-gray-300 mb-1">Description</label><textarea name="description" defaultValue={editing.description} rows={4} className="w-full px-4 py-2.5 rounded-xl bg-space-900 border border-[#2a2a2a] text-white text-sm focus:border-accent focus:outline-none transition resize-none" /></div>
